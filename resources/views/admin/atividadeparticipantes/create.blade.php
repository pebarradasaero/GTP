...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.atividadeparticipantes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Atividadeparticipante::PETISCO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('petisco') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="petisco_{{ $key }}" name="petisco" value="{{ $key }}" {{ old('petisco', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Atividadeparticipante::BEBIDA_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('bebida') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="bebida_{{ $key }}" name="bebida" value="{{ $key }}" {{ old('bebida', '') === (string) $key ? 'checked' : '' }} required>
