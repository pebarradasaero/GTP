...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.registoRegularidade.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.registo-regularidades.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RegistoRegularidade::REGULARIDADE_1_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('regularidade_1') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="regularidade_1_{{ $key }}" name="regularidade_1" value="{{ $key }}" {{ old('regularidade_1', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RegistoRegularidade::REGULARIDADE_2_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('regularidade_2') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="regularidade_2_{{ $key }}" name="regularidade_2" value="{{ $key }}" {{ old('regularidade_2', '') === (string) $key ? 'checked' : '' }} required>
