        ...
        <div class="col-md-12">
            @can('juntas_freguesium_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.juntas-freguesia.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $juntasFreguesium->id }}">
                                        ...
                                        <td>
                                            @can('juntas_freguesium_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.juntas-freguesia.show', $juntasFreguesium->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('juntas_freguesium_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.juntas-freguesia.edit', $juntasFreguesium->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('juntas_freguesium_delete')
                                                <form action="{{ route('frontend.juntas-freguesia.destroy', $juntasFreguesium->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('juntas_freguesium_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.juntas-freguesia.massDestroy') }}",
