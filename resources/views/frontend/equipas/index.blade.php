        ...
        <div class="col-md-12">
            @can('equipa_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.equipas.create') }}">
                            ...
                            <tbody>
                                @foreach($equipas as $key => $equipa)
                                    <tr data-entry-id="{{ $equipa->id }}">
                                        ...
                                        <td>
                                            @can('equipa_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.equipas.show', $equipa->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('equipa_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.equipas.edit', $equipa->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('equipa_delete')
                                                <form action="{{ route('frontend.equipas.destroy', $equipa->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('equipa_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.equipas.massDestroy') }}",
