    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.registo-regularidades.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\RegistoRegularidade::REGULARIDADE_1_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="regularidade_1_{{ $key }}" name="regularidade_1" value="{{ $key }}" {{ old('regularidade_1', '') === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\RegistoRegularidade::REGULARIDADE_2_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="regularidade_2_{{ $key }}" name="regularidade_2" value="{{ $key }}" {{ old('regularidade_2', '') === (string) $key ? 'checked' : '' }} required>
